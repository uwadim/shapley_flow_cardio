...
class CausalLinks:
    ...
    def add_causes_effects(self, causes, effects, models=None):
        '''
        ...
        assert len(models) in [0, len(effects)], f"must specify {len(effects)} models"
...
class Graph:
    ...
    def add_links(self, links):
        ...
        for causes, effects, models in links.items:
            ...
            for i, e in enumerate(effects):
                ...
                e.f = models[i] if len(models) == len(effects) else None
    ...
    def draw(self, rankdir="BT", show_gpath=True, save_fpath: Optional[str] = None):
        '''
        ...
        if save_fpath is not None:
            save_graph(self.to_graphviz(rankdir=rankdir), save_fpath)
    ...
    def sample_all(self, sampler):
        ...
        '''
        return {name: self.sample(sampler, name) for name in sampler}
...
class Node:
    ...
    def __init__(self, name, f=None, args=[],
        ...
        self.activated_args = {arg: False for arg in args}
...
class CreditFlow:
    ...
    def __init__(self, graph, verbose=False, nruns=10,
        ...
        self.node2sorted_idx = {
            node: i for i, node in enumerate(self.sorted_nodes)
        }
    ...
    def draw(self, idx=-1, max_display=None, format_str="{:.2f}",
             edge_credit=None, show_fg_val=True, save_fpath: Optional[str] = None):
        '''
    ...
    def get_asv_edge_credit(self, idx=-1, aggregate=True):
        ...
        for node1, d in flow_credit.items():
            for node2, val in d.items():
                ...
                if isinstance(idx, Iterable) and aggregate:
                    edge_credit[node1][target_node] += np.mean(np.abs(val[idx]))
                elif isinstance(idx, Iterable) or idx >= 0:
                    ...
                    if node not in G:
                        if node.is_noise_node and self.fold_noise:
                            ...
                            if idx < 0 or \
                                ...
                                fmt = "{}: {}" if isinstance(txt, str) else "{}: " + format_str
...
class GraphExplainer:
    ...
    def prepare_graph(self, X):
        ...
        self.graph.baseline_sampler.update(
            {
                name: self._idx_f(i, lambda i: bg[:, i])
        ...
        self.graph.target_sampler.update(
            {
                name: self._idx_f(i, lambda i: fg[:, i])
                for i, name in enumerate(names)
            }
        )
...
class ParallelCreditFlow:
    ...
    def run_subprocess(self):
        ...
        # join
        while True:
            job_status = [p.poll() is None for p in procs]
            if sum(job_status) <= 0:
                break
        ...
        name2node = {node.name: node for node in self.cf.graph}
...
def check_unique_node_names(graph):
    ...
    '''
    return len({n.name for n in graph}) == len(graph)
...
def get_source_nodes(graph):
    indegrees = {node: len(node.args) for node in graph}
    return [node for node in graph if indegrees[node] == 0]
...
def flatten_graph(graph):
    ...
    for node in topo_sort(graph):
        ...
        if not node.is_target_node and node.args != []:
...
def run_divide_and_conquer_set(graph, k=-1, verbose=False, len_bg=1):
    ...
    sources = get_source_nodes(graph)
    sorted_nodes = topo_sort(graph)
    node2sorted_idx = {n: i for i, n in enumerate(sorted_nodes)}
    ...
    def run(node, level, len_bg, update_root=None):
        ...
        def save_state(node, state):
            ...
            for c in node.children:
                ...
                state[c]['activated_args'] = dict(c.activated_args.items())
                state[c]['val'] = c.val
                save_state(c, state)
    ...
    def run(node, level, len_bg):
        ...
        def save_state(node, state):
            ...
            for c in node.children:
                ...
                state[c]['from_node'] = c.from_node
                state[c]['last_val'] = c.last_val
                state[c]['visible_val'] = dict(c.visible_arg_values.items())
                state[c]['val'] = c.val
                save_state(c, state)
...
# sample graph
def build_feature_graph(X,
                        causal_links,
                        categorical_feature_names=None,
                        display_translator=None,
